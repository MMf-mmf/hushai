...
Fixtures: `asr_short, two_speakers, jfk_moon, fdr_infamy, armstrong_step` (train) + `silence_no_speech`
(holdout). **Vision (faces/objects/plates) has scorers (`src/score.rs`) but NO Tier-1 fixture yet** —
it's only exercised via `probe`/physical capture. Adding a committable object fixture (PD car image →
`make_object_clip.sh`/`fetch_eval_clips.sh` recipe) is the top open gap; the object-decode fix below is
currently guarded by a unit test + the `probe` loop, not a Tier-1 case.
...
- ◐ **Plates — OCR DONE, detector remaining (2026-06-30).** fast-plate-ocr's API had moved
