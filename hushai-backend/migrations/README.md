...
| 0029 | `entity_baselines_digests` | Gotham Wave 2: `entity_baselines` (168 hour-of-week histogram, dwell p50/p90, device/companion stats, recomputed over a trailing window — PR4) + `daily_digests` (structured `sections` facts + deterministic `rendered_text`, no LLM at write time; produced by `patterns::build_and_upsert_digest` via `POST /v1/graph/digests/{date}` or the worker-0 wall-clock driver — PR5; LLM narrates at read time). DERIVED/rebuildable. |
...
| 0031 | `gotham` | Gotham G3 "Detective" agentic runtime: `chat_messages.tool_trace` (per-assistant-turn tool trace — shape not payloads; NULL elsewhere) + `gotham_pending_actions` (two-phase mutate-confirmation outbox, ≤ 1 pending per session via a partial-unique index). Backs `hushai-rag/src/gotham/`; read-only Phase 1 writes only `tool_trace`. |
