...
- [x] Android: voice keyword route (`AssistantRouting.kt` seam adds `Detective`/`DetectiveBare` on "detective" + `inspector`/`sherlock` Vosk fallbacks), `AWAIT_FOLLOWUP` confirmations (shared window, `followupAgent` dispatch), SSE unknown-event tolerance (`DetectiveClient` ignores `phase`/`tool_*`/`sources`) — **Phase H code + unit + live SSE-contract DONE** (built on the advisor-v2 Android voice seam, PR3 `32beb77`). `DetectiveClient` rides `/v1/rag/chat` `agent_id="gotham"` + voice caller (reuses rag host/token — no 2nd port); `HUSHAI_TX` markers added (advisor markers kept byte-identical). Unit 116/0; live: the real Detective runtime accepts the exact client body + streams the superset SSE (`session·phase·tool_call·tool_result·token·sources·done`). ◑ The phone-mic acoustic loop (Vosk hearing "detective" + spoken TTS marker order) is the remaining rig-gated step (same class as the advisor's own unrun phone phases).
...
- [x] Journey stitching in the events drain (open/closed contract from `conversations`) — `patterns::stitch_and_upsert_journeys`, called in `graph_pass` step-4 for every touched person/plate; `dedup_key = journey:<subject_id>:<first_hop_start_ns>` idempotent upsert; `closed` once tail < `now-(journey_gap+grace)`. `merge_in_tx` drops a loser's journeys. NO new hashed knob (`journey_gap_secs` already in `config_hash`).
- [x] `/v1/graph/journeys` + `/v1/graph/path` (shipped in G1); **journey tool for G3** — Detective `graph_journeys` (tool 21: resolve name → `/v1/graph/journeys?subject=`, renders the ordered camera path; no-subject fallback → recent journeys system-wide); timeline strip for G4 (Phase G — **still open**)
- [x] Fixture F2 journey assertions promoted to gating (`expect_journey {subject, cameras[]}` — ordered camera subsequence; F2 stitches a real front→garage journey; F10 agent-narrates it via `graph_journeys`)

**Shipped means**: "walk me through what Casey did yesterday" yields an ordered camera-hop narrative ("front door 09:02 → garage 09:07") with deep links; `expect_journey` green. **Producer + Detective tool + eval gate DONE + live-verified (F2 graph gate ×2 PASS; F10 agent `graph_journeys` PASS; `graph_db` guard: 2-device journey stitched, single-device skipped). The narrative surface (viewer timeline strip / deep links) is G4 / Phase G.**
...
| 21 | `graph_journeys` | entity_name (optional → recent) | read | `GET /v1/graph/journeys?subject=` — **SHIPPED (G5)**; renders the ordered camera path, no-subject → recent journeys |
...
- **Voice**: keyword invocation per the advisor-v2 pattern — "⟨wake⟩ **detective** ⟨question⟩" routes with `agent_id:"gotham"` + `caller:{kind:"voice", owner_verified, device_id}`. Risk note: "detective" must be validated against the Vosk small model exactly as "advisor" was (the v2 spec's one hard external dependency); fallback candidate words: "inspector", "sherlock". Voice clients ignore `phase`/`tool_*`; `confirm` → `AWAIT_FOLLOWUP`. **LANDED (Phase H):** `AssistantRouting` accepts all three keywords (so whichever the small model hears reliably routes — the "adviser" hedge); `DetectiveClient` (net sibling of `RagChatClient`) speaks only the answer and ignores every trace event; the `confirm` path is present-but-dormant (mutations off in Phase 1), mirroring the viewer's confirm bubble; the acoustic "detective" recognizability is the one piece that still needs the phone-mic rig.
...
| `expect_journey {subject, cameras[]}` | graph | ordered subsequence of a single journey's camera hops — **SHIPPED (G5)** |
...
| F2 | `graph_cross_camera_fusion` | train | same subject: front cam, garage cam +180s (within-gap → a stitched journey), + garage day 2 | `visits_place` edges to both devices; **`expect_journey [eval-graph-front, eval-graph-garage]` (G5, gating)** |
...
**Phase H — Voice (Wave 3). ◑ Code + unit + live SSE-contract DONE; phone-mic acoustic loop rig-gated.** Logcat marker contract under `HUSHAI_TX` (grep-stable strings: keyword routed, owner verified, confirm spoken, `AWAIT_FOLLOWUP` entered/expired) + phone rig loop on the phys stack (`hushai_test_phys`, +2 ports). **PASS:** marker order + DB shape. Delivered: `AssistantRouting` `Detective`/`DetectiveBare` (keyword "detective" + `inspector`/`sherlock` fallbacks, token-0 only, disjoint from advisor); `DetectiveClient` (net sibling of `RagChatClient`) → `/v1/rag/chat` `agent_id="gotham"` + voice caller, superset-SSE tolerant (ignores `phase`/`tool_*`/`sources`), `Answer`/`Confirm`/`SessionNotFound`/`EndpointMissing`/`Error`; `VoiceAssistant` detective consult loop reuses the shared `AWAIT_FOLLOWUP` window via a new `followupAgent` dispatch (advisor markers unchanged); detective session (`detective_session_*` DataStore, 30-min idle). Markers added: `detective route (session=…)`, `detective route: awaiting question`, `detective owner verified`, `detective answer ok (session=…)`, `detective confirm spoken (session=…)`, `detective confirm — awaiting yes/no`, `detective followup captured`, `detective followup timeout — resuming listen`, `detective error: …`. Unit **116/0** (`AssistantRoutingTest` +7 = 15, new `DetectiveClientTest` 10 incl. the unknown-event-tolerance + confirm-outranks-answer proofs). Live-verified: the real `--test-db` Detective runtime accepted the exact client body and streamed `session·phase·tool_call·tool_result·token×24·sources·done` (an `ask_user` turn — the clarifying question rode back as the answer, exactly the Phase-1 web behavior). **Remaining rig-gated:** install on the phys stack + enrolled owner + speak "⟨wake⟩ detective …" → assert marker order + `chat_sessions`/`chat_messages` shape; and the acoustic Vosk validation of "detective" (fall back to `inspector`/`sherlock` if it mishears — the code already routes all three).
...
| H | Voice — Android "detective" keyword route + `AWAIT_FOLLOWUP` confirmations + SSE unknown-event tolerance. **Code + unit (116/0) + live SSE-contract DONE** (`AssistantRouting` `Detective` + `inspector`/`sherlock` fallbacks; `DetectiveClient` → `/v1/rag/chat` `agent_id="gotham"` voice caller, superset-tolerant; `VoiceAssistant` detective consult reuses the shared `AWAIT_FOLLOWUP` via `followupAgent`, advisor markers byte-identical; `HUSHAI_TX` detective markers; `detective_session_*` DataStore). Live: real `--test-db` Detective accepted the exact client body + streamed the superset SSE. **Rig-gated remainder:** phone-mic acoustic loop (Vosk hears "detective" + spoken TTS marker order + DB shape) — same class as the advisor's own unrun phone phases. | ◑ |
...
10. **Voice route + confirmations + phone rig** — Phase H. ◑ **Code + unit + live SSE-contract LANDED** (built on the advisor-v2 Android voice seam, PR3 `32beb77`): `AssistantRouting` `Detective`/`DetectiveBare` (keyword "detective" + `inspector`/`sherlock` fallbacks), `net/DetectiveClient.kt` (net sibling of `RagChatClient` → `/v1/rag/chat` `agent_id="gotham"` + voice caller, superset-SSE tolerant, `Answer`/`Confirm`/`SessionNotFound`/`EndpointMissing`/`Error`), `VoiceAssistant` detective consult loop reusing the shared `AWAIT_FOLLOWUP` window via a new `followupAgent {NONE,ADVISOR,DETECTIVE}` dispatch (advisor marker strings unchanged), `detective_session_*` DataStore (30-min idle, reuses rag host/token — no new port/URL/extra), `HUSHAI_TX` detective markers. Unit **116/0** (`AssistantRoutingTest` 15 + new `DetectiveClientTest` 10). Live: the real `--test-db` Detective runtime accepted the exact client body + streamed the superset SSE (an `ask_user` turn). **Remaining rig-gated: the phone-mic acoustic loop** (install on the phys stack, enrolled owner, speak "⟨wake⟩ detective …", assert marker order + DB shape; acoustic Vosk validation of "detective").
