...
| `hushai-android/` | Native Android capture client (Kotlin). Camera2 + dual MediaCodec → ~2s segments → uploads; live preview, battery-saver, voice assistant, an **audio-only mode** (mic-only FGS, no camera), an **opt-in upright-BAKE mode** (`Settings.uprightBake` / `--ez upright_bake true`: a GLES pass rotates the encoded pixels upright on-device so even the viewer's fast `-c copy` plays upright — falls back to the rotation-matrix path if GL init fails; rotation sign is rig-empirical), and a **Voices screen** (`ui/VoicesScreen.kt` + `net/SpeakersClient.kt`: list/name/merge speakers, play a sample-audio snippet, plus a **"Clean up voices"** section that surfaces backend-suggested duplicate groups and merges them in one tap / "Merge all"), and a **People screen** (`ui/PeopleScreen.kt` + `net/PersonsClient.kt`: the visual twin of Voices — list/name/merge faces, showing each face's sample-face crop via OkHttp + `BitmapFactory` + Compose `Image`, no image lib), and a **Plates screen** (`ui/PlatesScreen.kt` + `net/PlatesClient.kt`: the vehicle twin of People — list/search/name/merge license plates, showing each plate's `GET /v1/plates/{id}/sample-crop` crop the same OkHttp+`BitmapFactory` way; unnamed plates are labelled by their OCR `plate_text`). All three screens carry a **Disregard** button per card + a collapsed **"▸ Archived (N)"** section with Restore (`setArchived` on each net client). These are screen-state toggles in `MainActivity` (`Screen.{Capture,Voices,People,Plates}`), no nav framework. **First real client.** See its `README.md`. **Voice Q&A now uses the rich `/v1/rag/chat` (2026-07):** `net/RagChatClient.kt` consumes the SSE stream (accumulates `token` deltas; parser ported from `hushai-eval/src/query_rag.rs`), so the assistant gets the server auto-router (replacing on-device `ReflectionIntent`), sessions + condensation, and the identity/recency/context features. `assistant/VoiceSession.kt` persists the `session_id` in DataStore with a 15-min idle window so spoken follow-ups continue the conversation. The request sends `caller:{kind:"voice", owner_verified}` (the on-device voiceprint result — only claimed when actually enrolled + matched), the phone's real `tz_offset_secs`, and **no `filters.device_id`** (voice searches all devices). The legacy single-shot `RagClient`/`ReflectionIntent` are retained ONLY as the fallback when the server 404s the chat endpoint. VoicesScreen gained a **"This is me"** action (`SpeakersClient.setOwner` → `/v1/speakers/{id}/owner`). | ✅ built + **verified E2E on a physical Galaxy S8** |
...
**hushai-viewer gotcha — upright video (rotation):** Android stamps an MP4 rotation matrix
(`setOrientationHint`), but a `-c copy` remux to MPEG-TS drops it (TS can't carry it; hls.js/MSE ignore
container rotation) → sideways playback. `remux.rs` therefore `ffprobe`s each video's display-matrix
rotation and, when non-zero, re-encodes it UPRIGHT with libx264 (ffmpeg autorotate — no `-vf transpose`),
baking the rotation into the pixels; 0°/matrix-less clips keep the fast `-c copy` path. Toggle
`VIEWER_UPRIGHT_REENCODE`; purge `cache/ts/` once on deploy. The Android app can also bake pixels at the
source (opt-in `Settings.uprightBake` → `capture/gl/` + `GlVideoPipeline`), which zeroes the matrix so
the fast copy path plays it upright with no re-encode.
...
## Worker efficiency: skipped status / ingest hint gate / skip-silent / skip-static / load governor (read before touching `db.rs::persist_segment`, `process.rs`, `vision/write.rs`, or the `lib.rs` loops)

These measures cut wasted compute and keep the device from ever being driven into overload. All default ON with conservative, **uncalibrated** thresholds + a kill switch each (`hushai-worker/.env.example`, `hushai-backend/.env.example`); the durable queue still guarantees nothing is ever dropped — media is ALWAYS stored regardless of gating.

- **`skipped` status (migration 0022)** — both lane status tables are `pending | processing | done | error | skipped`. `skipped` is TERMINAL: a content gate decided there is nothing to infer. `skip_reason` names the decider (`silent_hint`/`static_hint` = ingest gate from device hints; `silent_gate`/`static_gate` = worker backstop). Never claimable (claim SQL + the 0020 partial indexes enumerate claimable statuses) and — deliberately — invisible to `reconcile_missing_speaker_segments` (matches `done` only), so skips are never resurrected on restart. Re-evaluate after a calibration change with the explicit flip:
  `UPDATE segment_transcription_status SET status='pending', attempts=0, skip_reason=NULL, claimed_at=NULL, updated_at=now() WHERE status='skipped'; NOTIFY hushai_segment_ingested, 'reprocess';` (same for `segment_vision_status`).
- **Ingest hint gate (backend `hints.rs` + `db.rs::persist_segment`)** — the Android app attaches RAW per-segment measurements as contract-§8 attrs (`hint.v=1`, `hint.audio_rms`, `hint.audio_peak_rms` from `AudioEncoder`; `hint.motion_score`/`hint.motion_fp_side` from `MotionHintAnalyzer`, a 320×240 ImageReader mirroring the worker's 32×32 mean-subtracted-MSE fingerprint). The backend owns ALL thresholds (`INGEST_HINT_GATE_ENABLED`, `INGEST_AUDIO_RMS_FLOOR=0.005` on peak-RMS, `INGEST_MOTION_THRESHOLD=4.0` — half the worker's, different pixel path): below-floor lanes are INSERTED pre-terminated `skipped` (not suppressed — the worker's `ensure_status_rows` backfill would resurrect a missing row) and don't `pg_notify`. **Fails open** on absent/malformed/unversioned hints (web capture, loadtest, old app builds ⇒ legacy path); branches only on hint PRESENCE, never `source_kind` (§7). **Audit sampling**: `INGEST_HINT_AUDIT_PCT` (2%) of would-be skips are enqueued `pending` with `hint_audit=true`; the worker's own gate records `audit_verdict='agree'|'disagree'` — the dashboard warns when disagreement >10% (hints miscalibrated/lying). Counters: `hushai_ingest_segments_skipped_total{lane,reason}`, `hushai_ingest_hint_audit_enqueued_total{lane}`, `hushai_worker_hint_audit_total{lane,verdict}`.
- **Skip-silent (audio)** — `process::process_segment`, right after `extract_pcm`, before `transcribe`. Two stages, cheapest first: a free `vad::rms` floor (`AUDIO_SILENCE_RMS_FLOOR`) short-circuits dead air, else the already-loaded Silero VAD's `speech_secs` vs `AUDIO_SILENCE_MIN_SPEECH_SECS` makes the call. A silent segment writes NOTHING (no empty transcript, **no reject tombstone** — the `skipped` status itself keeps the reconcile away) via `write_skip`: one tx clearing prior sentences + marking `skipped/silent_gate` with `measured_rms`/`measured_speech_secs` telemetry. Tombstones remain only on the processed-but-no-voice path (`write_transcript(…, speaker=None)`, still `done`). **Fails OPEN** (VAD error ⇒ normal ASR). Pure decision is `vad::silence_verdict` (unit-tested, no model). Counter: `hushai_audio_segments_skipped_silent_total{reason=rms|vad}`. Does NOT affect speaker windowing (neighbors are read by sequence, blobs/status intact).
- **Skip-static (vision)** — `vision::write::process_vision_segment`, before the model loop. Cross-segment motion via `vision/motion.rs`: a 32×32 mean-subtracted-MSE grayscale fingerprint per camera (`VISION_MOTION_FP_SIDE`), cached **in-memory** on `VisionModels.motion_cache` (`Arc<Mutex<HashMap<device_id, Fingerprint>>>`, one map/process, lossy-on-restart by design). With `WORKER_GATE_ONE_FRAME_PROBE=true` (default) the gate decodes ONE mid-segment frame first and pays the full `FRAMES_PER_SEGMENT` decode only after motion confirms (~1/3 static decode cost). `distance <= VISION_MOTION_THRESHOLD` ⇒ skip ALL face/object/plate inference, write no detection rows, return `VisionOutcome::SkippedStatic` ⇒ the loop marks `skipped/static_gate` (+ `measured_motion_distance`); cache is updated on both branches (drift re-baselines, PTZ self-corrects). Skip writes nothing on purpose — presence continuity comes from the 30s event sessionization, NOT carry-forward (which would fabricate detections + pollute the k-NN match substrate). Counter: `hushai_worker_segments_skipped_static_total{lane=vision}`. **Set `VISION_MOTION_SKIP_ENABLED=false` for any full reprocess/calibration run** (the per-camera baseline is meaningless replaying old segments out of order).
...
1. **Self-healing speaker backfill** (`hushai-worker/src/{claim,lib,config}.rs`):
   ...
   Matches `done` ONLY — `skipped` segments (content gates, migration 0022) are immune by
   status, which is why the silent-skip path no longer writes any tombstone. For segments that
   WERE processed but resolved no voice, convergence depends on the **reject tombstone** below —
   such a segment writes a `quality='reject'` row (NULL speaker, NULL embedding) so it isn't
   re-queued every restart; `assign_speaker` ignores that tombstone so a later, better run
   (e.g. windowed) can still attribute it. Tombstones are inert to matching/clustering/centroid
   (all filter them out).
...
- **Viewer "Detections" overlay — built + verified E2E (the headline of this work):** the browser NVR
  ...
  displayed letterbox scaling against `video.videoWidth/Height` — sound because the playback dims match the
  worker's autorotated detection-frame dims: a 0° clip stays `-c copy` at source dims, and a rotated clip is
  re-encoded UPRIGHT (see the upright gotcha below) so both sides agree — the rotated case that used to be
  misaligned is now correct — snap-to-nearest-sampled-frame within tolerance). Verified in real headless
  Chrome (boxes track the picture).
- **Viewer scrub-bar AI processing-status ribbons — built + verified E2E:** the timeline shows how far the
  ...
  (`done`/`processing`/`pending`/`error`/`skipped`; `processing` animates a marching shimmer; `skipped` =
  the migration-0022 terminal content-gate verdict, rendered dim gray-green + "(static)"/"(silent)" in the
  tooltip and ranked below `done` in the badge), plus a live
