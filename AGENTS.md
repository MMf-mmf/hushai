...
| `hushai-viewer/` | The **unified webapp** (`127.0.0.1:8070`): scrubbable HLS NVR **+ chat panel** (slash-picker plugins: `auto` recordings · `advisor` book-grounded consults · `gotham` the Detective) + **Investigate page** (`investigate.html`: entity/link explorer, cross-camera journey strips, voice↔face binding review queue) + admin modals (Voices/People/Plates) + System dashboard + Events/Files pages. Reverse-proxies `/v1/*` to **three** upstreams by path (rag · backend · advisor), one browser origin, server-side bearer. Also a capture **source** (browser camera/mic). Vanilla ES modules, **no build step**. | [`hushai-viewer/README.md`](hushai-viewer/README.md) |
